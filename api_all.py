...
===== C:\Users\rober\OneDrive\문서\vscode\AutoTrader_Project\api\auth.py =====
...
===== C:\Users\rober\OneDrive\문서\vscode\AutoTrader_Project\api\candle_cache.py =====
...
===== C:\Users\rober\OneDrive\문서\vscode\AutoTrader_Project\api\kiwoom_rest.py =====
...
class KiwoomREST:
    ...
    def get_minute_candles(self, stock_code: str, interval: int = 1,
        ...
        """遺꾨큺 議고쉶 (罹먯떆 寃쎌쑀). 8珥?TTL + 429/?ㅽ뙣 ??吏곸쟾 罹먯떆 諛섑솚."""
...
===== C:\Users\rober\OneDrive\문서\vscode\AutoTrader_Project\api\kiwoom_ws.py =====
...
class KiwoomWS:
    ...
    async def _dispatch_orderbook_item(self, item: dict):
        ...
        if not getattr(self, "_orderbook_keys_logged", False):
            logger.info(f"?뵎 0D ?멸? raw ?? {list(values.keys())}")
            self._orderbook_keys_logged = True

        # FID 留ㅽ븨 (?ㅼ? 0D ?쒖? 異붿젙 ??raw ??濡쒓렇濡?寃利????섏젙)
        ...
        try:
            ...
            logger.exception(f"on_orderbook 肄쒕갚 ?덉쇅: {stock_code}")
    async def _dispatch_condition_item(self, item: dict):
        stock_code = (
            item.get("9001") or item.get("jmcode") or item.get("stk_cd") or ""
        ).strip()
...
===== C:\Users\rober\OneDrive\문서\vscode\AutoTrader_Project\api\test_minute_candles.py =====
...
===== C:\Users\rober\OneDrive\문서\vscode\AutoTrader_Project\api\ws_supervisor.py =====
