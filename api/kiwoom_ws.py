...
# ─────────────────────────────────────────
# HTS [0156] 조건검색 화면의 '대상' 설정과 **맞춰 두는 것이 안전하다.**
# 어긋나도 에러가 나지 않고 조용히 0종목이 될 수 있는 부류라, 한쪽만 바꾸면
# 하루 종일 매수 0건이 되고도 로그에 아무 흔적이 없다.
#
# ── 2026-08-07: 통합("A") -> KRX("K")로 되돌림 (사용자 지정) ──────────
# [왜] **계층마다 거래소 기준이 달라 유니버스와 판정이 어긋나 있었다.**
#   조건검색(편입)      = 통합(KRX+NXT)   <- 08-05에 여기만 바뀌었다
#   실시간 체결틱 0B     = **KRX만**       <- 무장·버스트·VWAP의 근간
#   REST 분봉/현재가     = **KRX만**       <- 시가·전일종가·등락률
#   주문/잔고            = **KRX만**       (dmst_stex_tp="KRX")
#   즉 '통합 기준으로 고른 종목'을 'KRX 데이터로만' 판정하고 있었다.
#
# [실측] 키움은 거래소를 종목코드 접미사로 나눈다(`005930` / `_NX` / `_AL`):
#     금호타이어 073240  KRX 19,927,188 + NXT 20,347,524 = 통합 40,274,712
#     -> NXT 상장 종목에서 우리가 못 보는 거래량이 **평균 54%**다.
#     시가도 다르다(KRX 7,200 vs 통합 7,150).
#   실시간 0B가 KRX 전용임은 틱 아카이브로 확정했다 — 073240의 09:00~09:15
#   0B 틱 합 1,919,958 = KRX 분봉 1,919,958 (정확히 일치).
#
# [실측] 2026-08-07 02:33 조건식별 A vs K (장 마감 상태):
#     주도주상위     A=6  K=7   <- K에만 073240
#     돌파자동매매용  A=0  K=1   <- K에만 033160
#     눌림목자동     A=3  K=3   <- 동일
#     종가베팅       A=19 K=14
#   **통합이 유니버스를 넓혀주는 게 아니라 '다른 유니버스'다.** 매매용 3개
#   기준으로는 오히려 K가 2종목 많았다(9 vs 11).
#
# ⚠️ [정정] 08-05 08:33에 "K -> 0종목"이던 것은 HTS 설정 불일치가 아니라
#   **그 시각에 KRX가 안 열려 있었기 때문**일 가능성이 크다. NXT는 프리마켓
#   08:00~08:50이 있어 08:33엔 NXT만 거래 중이다. 위 02:33 실측에서 K가
#   정상 동작(7종목)한 것이 그 근거다. -> **08:59 기동 스냅샷이 0종목일 수
#   있다**(KRX 개장 1분 전). 09:00부터 실시간 편입이 받으므로 매매는 계속된다.
#
# 되돌리려면 이 값을 "A"로 바꾸고 HTS 대상도 통합으로 되돌릴 것.
CONDITION_STEX_TP = "K"   # "A"=통합(KRX+NXT) / "K"=KRX / "N"=NXT
