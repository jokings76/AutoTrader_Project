...
#
# ✅ **2026-08-07 장마감 후 ON** (사용자 지정). 그날 실거래 10건에 소급하니
#    손실 2건을 정확히 걸렀다 — 심텍 09:12(이격 -2.01%, 실현 -1.12%) /
#    샘씨엔에스 09:59(-1.18%, -3.03%). 가격기준 -4.35% -> -0.20%,
#    실현금액 -42,120원 -> **-17,110원**(개선 +25,010원).
#
# ⚠️ **근거는 여전히 얇다. 반드시 같이 기억할 것:**
#    · 4일 실거래 69건으로는 손익비 1.03 -> **1.09**에 그쳤고, +1.0% 이상
#      구간은 오히려 0.80으로 악화돼 단조성이 깨진다.
#    · 08-07 소급은 n=10 **하루치**다. 이 프로젝트가 반복해서 밟은 함정이
#      정확히 이것이다(08-06 최적 파라미터가 4일로는 최악이었다).
#    · **만능이 아니다** — 08-07 이랜시스는 이격 +2.36%로 통과했는데
#      -3.11% 손절이었다. 손실 3건 중 1건은 못 잡는다.
#    · 차단된 2건을 안 샀으면 **슬롯이 비어 다른 종목을 샀을 수도** 있는데
#      그 반사실은 재현되지 않는다.
#    나빠지면 VWAP_ENTRY_ENABLED = False 한 줄로 즉시 복귀.
#
...
VWAP_ENTRY_ENABLED = True           # False로 되돌리면 즉시 해제된다
...
# ✅ **단위 확정 (2026-08-07 09:00:06 실거래)** — FID 14는 **백만원 단위**다.
#    _note_session_vwap()의 진단 로그 실측:
#      [460940] FID13(누적량)=7,649 / FID14(누적대금)=43 / 현재가=5,640원
#    실제 거래대금 = 7,649주 x 5,640원 = 43.14 백만원 vs FID14=43 (오차 0.33%).
#    가설별 비율(=VWAP/현재가): 원 0.0000 / 천원 0.0010 / **백만원 0.9967** /
#    억원 99.67 -> **1_000_000.0만 자릿수가 맞는다.**
#
# ⚠️ FID 14는 백만원 단위 **정수**라 값이 작으면 양자화 오차(±0.5/값)가 커진다.
#    판정 문턱(VWAP_ENTRY_MIN_GAP_PCT=0.5%)보다 커지면 판정이 무의미해지는데,
#    VWAP_ENTRY_FROM(09:05)이 그 문제까지 같이 막아준다 — 08-06 실측 누적:
#      코스모 455억(FID14=45,495, 오차 0.001%) / 대원전선 273억(0.002%) /
#      **현대약품 5.78억(FID14=578, 오차 0.086%)** <- 가장 얇은 종목도 문턱의 1/6.
#    09:05를 앞당기려면 이 오차를 다시 계산할 것.
...
# (2026-08-06 사용자 지정) 2.00 -> **3.00**. "고가주는 문턱을 확실히 올려라".
# [08-06 실측 근거] 3만원 이상 6건 평균 **-1.46%**, 승률 1/6. 그중 10만원대
# 3종목(심텍·주성·에스피지)이 계수 2.00(문턱 1.6억)에 걸려 있었는데, 실제
# 버스트는 **단일체결 2.04~7.69억**으로 문턱을 너무 쉽게 넘었다.
#   심텍 113,900 단일 2.93억 -> 새 단일문턱 3.00억에 **탈락**(그 건은 -3.07%)
#   주성 141,100 단일 2.04억 -> 탈락
# [부수효과 없음 — 이게 3.00을 고른 이유]
#   클램프 시작가가 35,264원 -> **73,704원**으로 올라가, 영향받는 건 7.4만원
#   이상뿐이다. 08-06 거래에 소급하면 **영향받는 4건이 전부 손실/0%**이고
#   (주성 0.00 / 에스피지 -3.06 / 심텍 0.00 / 심텍 -3.07, 합계 -6.13%),
#   **그날 승자 4건(코스모 +3.17 / 금호타이어 +2.85 / 현대약품 +4.25 /
#   GS건설 +0.46)은 전부 클램프 밖이라 그대로 통과**한다.
# ⚠️ [한계] 3만원대는 클램프 밖이라 **이 변경으로 안 바뀐다**(GS건설 32,600원은
#   계수 1.92 그대로, 그날 -3.07%였다). 그 구간까지 올리려면 ALPHA를 올려야
#   하는데, 그러면 2만원대까지 같이 조여져 그날 승자였던 코스모(20,500원,
#   버스트가 문턱에 딱 걸쳐 있었다)가 탈락한다. 그래서 일부러 MAX만 올렸다.
# ⚠️ **상한이 걸리기 시작하는 주가**를 반드시 같이 볼 것 — 계수가 MAX에 닿는
#    지점은 `10,000 x MAX^(1/ALPHA)`이라 ALPHA에 따라 크게 움직인다:
#      MAX=1.4 -> 18,437원~   MAX=1.6 -> 23,503원~   MAX=1.8 -> 29,116원~
#      MAX=2.0 -> 35,264원~   MAX=2.5 -> 52,909원~   MAX=3.0 -> **73,704원~**
#    즉 MAX는 **그 지점 이상에만** 효과가 있다. 그 아래 종목은 자기 자연계수를
#    쓰므로 MAX를 아무리 올려도 안 바뀐다.
BURST_PRICE_MAX = 3.00        # 상한 -> 약 73,704원 이상은 전부 1.2억/3억으로 수렴
...
# ── 매수 **주가 상한** (2026-08-13 신설, 사용자 지정) ──────────────────
# 이 가격 **이상**인 종목은 신규로 사지 않는다.
#
# [근거] 08-05 실전 전환 이후 청산 105건을 가격대로 갈랐다.
#   ⚠️ '수동 매도 감지'(매수가로 대체 기록된 손익 0)는 **가짜값이라 제외**했다.
#     가격대      청산n   평균      중앙값    손절률  -2%이하  최악
#     1만 미만      59   +0.16%   +0.23%    36%    22%   -5.65%
#     1~2만        22   -0.57%   -0.49%    45%    27%   -3.03%
#     2~5만        16   -0.14%   +0.20%    44%    25%   -3.07%
#     **10만+**     8   **-1.87%** **-2.08%**  **62%**  **50%**  -3.93%
#   · **-2% 이하 청산이 절반**이다(다른 밴드는 22~27%). 중앙값이 마이너스인
#     유일한 밴드이고, 플러스는 한미반도체 +0.30% 하나뿐이다.
#   · 🔴 **날짜 효과가 아니다** — 10만원+를 산 4일 **전부** 그날 평균보다 나빴다
#     (08-06 -3.06 vs -0.98 / 08-07 -0.74 vs -0.44 /
#      08-10 -1.15 vs -0.07 / 08-11 -3.93 vs -0.91). 4/4일 일관.
#     (이 프로젝트가 반복 경고해 온 "구간별 평균은 날짜로 쪼개 볼 것"을 지킨 값.)
#
# [🔴 구조적 원인 — 왜 비쌀수록 나쁜가]
#   진입 로그를 전수 파싱해 '발사 게이트가 사실상 없던 매수'의 비율을 냈더니
#   **가격과 단조 증가**했다: 1만미만 26% / 1~2만 28% / 2~5만 32% / **10만+ 50%**.
#   10만+ 6건의 내역:
#     ① **주가계수 절단 3건** — 버스트 문턱 계수가 clamp(..., 0.30, 3.00)라
#        **73,704원부터 상한에 걸린다.** 139,700원은 이론계수 4.26이 3.00으로
#        잘려 문턱이 의도보다 **30% 헐겁고**, 205,000원은 43% 헐겁다.
#     ② **가속도 4.00 = 수학적 상한 2건** — 편입 직후 ~90초는 120초 버퍼가
#        비어 무조건 상한이 된다(문서에 기록된 아티팩트). 게이트가 없는 것과 같다.
#     ③ 개장초반 발사면제 1건.
#   즉 "비싼 종목이 나쁘다"기보다 **비싼 종목에서 게이트가 헐거워진다**가 기전이다.
#
# [🔴 2026-08-13 장마감 후 MAE 전수 측정 — 위 '기전' 설명을 **정정**한다]
#   분봉으로 매수 후 최저/최고를 전수 측정(10만+ 12건 vs 2만미만 무작위 25건):
#     구분        n    MAE평균   MAE중앙   MFE평균   **MFE/MAE**  MFE3%+
#     10만원+     12   -5.03%   -5.00%   +2.28%     **0.45**     25%
#     2만원미만    25   -4.87%   -4.51%   +6.74%     **1.38**     64%
#   · 🔴 **하락폭은 거의 같다**(차이 0.16%p). "비싼 종목이 더 떨어진다"는
#     사실이 아니다 — 저가주도 76%가 -2% 이하로 밀린다(10만+는 83%).
#   · 🔴 **진짜 차이는 상승폭이다 — 3배**(+2.28% vs +6.74%). 10만원+는 같은
#     만큼 밀리는데 **되돌아오지 못한다.** MFE가 3% 이상 오른 비율이
#     25% vs 64%이고, 최대 MFE가 +7.34% vs +20.88%다.
#   => 이 상한의 근거는 '낙폭'이 아니라 **손익비(0.45 vs 1.38)**다.
#      손절을 넓히거나 보유를 늘리는 방향으로는 해결되지 않는다(올라오질 않는다).
#
# [부수 문제] 되돌림 1차 트랜치(약 40만원)가 양자화로 크게 남는다 —
#   105,550원 3주(21% 미집행) / 139,700원 2주(30%) / 205,000원 1주(**49%**).
#   포지션 사이징이 설계와 어긋난다.
#
# ⚠️ **표본 8건이다.** 이 프로젝트 기준(30건 + 3일 이상 우세)에 못 미친다.
#    방향이 4/4일 일관하고 구조적 원인이 코드로 확인된다는 점이 근거이고,
#    성과가 입증된 값이 아니다.
# ⚠️ **경계값 10만원은 데이터로 고른 값이 아니다.** 5~10만원 구간은 실거래
#    표본이 **0건**이라, '어디부터 나빠지는지'는 5만~10.5만 사이 어딘가로만
#    알 뿐 확정되지 않았다. 사용자가 고른 라운드 넘버다.
# ⚠️ 물타기(_maybe_average_down)는 bypass_entry_gates=True라 **이 상한을 받지
#    않는다**(모든 품질 게이트와 동일). 반면 rescue-add는 이 상한을 받으므로,
#    상한 도입 전에 산 10만원+ 보유분은 '손절 대신 추가매수'가 막히고 손절로
#    수렴한다 — 안전 방향이고, 상한이 켜진 뒤엔 신규 취득이 없어 곧 무의미해진다.
# 롤백: 0 -> 상한 없음 (08-12까지의 동작).
MAX_ENTRY_PRICE = 100_000
...
# ── 조건식별 숙성 예외 (2026-08-07 신설, 사용자 지정) ───────────────────
# [왜] 돌파자동매매용은 **개장 직후 돌파 순간**을 잡는 검색식이라 편입이
# 그 순간에 몰린다. 08-07 실측: 18종목 중 **14개(78%)가 09:00~09:02 편입**
# (주도주상위는 32종목 중 9개=28%). 그런데 60초 숙성이 정확히 그 구간을
# 통째로 덮어서, 이 검색식의 신호를 구조적으로 못 쓰게 만들고 있었다.
#
# [증거] 08-07에 숙성으로 막힌 4종목이 **전부 돌파자동매매용 최초 편입**이다:
#     원익홀딩스 09:00:16(55초<60초, 5초 차이로 탈락 -> 끝내 미매수)
#     후성       09:00:26(31초 -> 끝내 미매수)
#     금호타이어  09:01:02(76건 차단 후 나중에 매수)
#     이랜시스    09:55:58(15건 차단 후 나중에 매수)
#
# [층위별로 결론이 뒤집힌다 — 이게 이 예외의 근거다]
#   · 조건식 자체(편입 시점 매수 가정, 2일): 돌파 -0.46% > 주도 -1.13%
#     (08-06 돌파 +0.89 vs 주도 -0.21 / 08-07 돌파 -1.81 vs 주도 -1.78)
#   · 우리 실매수(실전 3일): 단독 주도 +0.76%(승60%) > 단독 돌파 -0.17%(승33%)
#   -> **조건식은 돌파가 나은데 우리가 산 결과는 주도가 낫다.** 그 간극을
#      만드는 게 숙성이라고 보고 돌파만 30초로 완화한다.
#
# ⚠️ **실거래 미검증이다.** [F] 60초는 4일 실측에서 "4/4 전부 개선"으로 이
#    프로젝트에서 근거가 가장 강했던 항목이고, 이 예외는 그 근거가 없는
#    구간(30~60초)을 여는 것이다. 08-07 실측만 봐도 숙성이 막은 2건은
...
# ── 개장 초반 슬롯 캡 (2026-08-08 신규, 사용자 지정) ───────────────────
# 위 '발사 게이트 면제'의 짝이다. 09:00~09:05에 발사 게이트가 없으므로
# **무장만 되면 되돌림 계획이 열린다** — 그 구간에 하루 28.8종목이 무장하는데
# (틱룰 강도 근사) 공유 슬롯은 6이라, 그냥 두면 **먼저 무장한 순서로** 자리가
# 다 차버린다. 08-06에 개장 18분 만에 19건을 사고 -138,152원을 낸 것과 같은 형태다.
#
# [🔴 왜 가속도로 막지 않았나 — 실측이 둘 다 반대였다]
#   ① **09:00~09:02는 가속도가 구조적으로 부풀려진다.** 120초 창인데 09:00
#      이전 데이터가 없어 실제로는 [0, t]만 본다. 균일 유입률 r이어도
#          accel = r*min(30,t) / (r*t/4) = 4*min(30,t)/t
#      이라 t<=48초면 무조건 2.5를 넘는다(09:01 2.00 / 09:02 1.00).
#      실측도 그대로였다 — 09:00부터 가속 2.5를 걸어도 발화가 하루
#      28.8 -> 28.2종목으로 **거의 줄지 않는다**(3.0으로 올려도 27.6).
#   ② **워밍업을 피해 09:01부터 걸면 발화는 줄지만 성과가 무너진다:**
#          게이트 없음  28.8종목  +0.896%  플러스일 5/5
#          가속2.5(09:01~) 11.8종목 +0.295%  3/5
#          가속3.0(09:01~)  7.2종목 -0.104%  2/5
#          가속2.5(09:02~)  8.2종목 -0.434%  2/5
#      문턱은 곧 '기다림'인데 초반 우위는 지연에 극도로 민감하다
#      (09:00:30 +0.896% -> 09:02 +0.372%). 나쁜 종목을 거르는 게 아니라
#      **좋은 시각을 통째로 버린다.**
#
# [그래서 개수로 막는다] 초반 28.8종목의 기대값이 전부 양수(5/5)라 걸러낼
# 나쁜 종목이 없다 — 이건 **신호 문제가 아니라 슬롯 배분 문제**다.
# 실측(되돌림 체결분 89건, 가속도 순위로 상위 K개만 산 경우):
#       상위 3개  +2.425%(4/5)   [대조 종목코드순 +0.337%]
#       상위 4개  +1.738%(4/5)   [대조 +0.754%]
#       상위 6개  +1.980%(4/5)   [대조 +1.876%  <- 6에서는 차이가 사라진다]
#       전부(17.8) +1.074%(5/5)
# 캡을 낮출수록 `_try_1a_priority_upgrade`(가속도 x 강도 tier)가 자주 돌아
# **사후 순위 배분에 가까워진다.** 4는 사용자 지정값이다.
#
# ⚠️ 위 순위 실측은 09:00~09:05 전체를 놓고 **사후에** 매긴 것이다. 실전은
#    먼저 무장한 종목이 먼저 슬롯을 잡으므로 그대로 재현되지 않는다 —
#    근사하는 장치가 candidate_tier 기반 교체다.
# ⚠️ 남는 2칸은 09:05 이후(가속 2.5 게이트가 도는 구간)를 위해 남겨두는 것이
#    이 설계의 의도다. 캡을 6으로 올리면 초반이 다 먹는다.
...
#
# 🔴 (2026-08-10 장중, 사용자 지정) **6 -> 0으로 껐다.** 실거래가 설계 전제를 깼다:
#   · 09:01:41~09:03:36, **115초 만에 6회를 전부 소진**했다(설계는 '하루 몇 번').
#     초반 슬롯 캡(4칸) 때문에 09:00~09:05에 can_buy_more가 상시 False가 되고,
#     후보가 쏟아지는 그 5분 동안 이 경로가 매 틱 열린 것이 원인이다.
#   · 그중 **3회는 유령 포지션(지정가 미체결)에 낭비**됐다 — 매도가 3번 실패하고
#     한도만 태웠다. 실제로 팔린 3건은 전부 '팔 이유가 없던' 정체 종목이다
#     (JW신약 -1,200 / 엑스게이트 -945 / 대한광통신 +540 = -1,605원 + 왕복수수료).
# ⚠️ 재시작하면 카운터가 0으로 리셋돼 같은 폭주가 재현되므로, 오늘 재시작 전에
#    상수 자체를 0으로 내렸다.
# 되살릴 때는 6이 아니라 **2~3** 또는 '09:05 이전 금지'를 먼저 검토할 것.
PHASE1A_PRIORITY_MAX_PER_DAY = 0
...
#   ⚠️ 1A에는 시가대비 매수보류(PHASE1A_LEADING_OPEN_SURGE_CAP)가 따로 있어
#      둘 중 **낮은 쪽이 먼저 걸린다**. 도입 당시엔 보류가 5%라 그쪽이 먼저였지만,
#      08-06에 8%로 완화되면서 지금은 이 VI 차단(6.80%)이 먼저다.
#      눌림목자동엔 시가대비 필터가 없으므로 Pullback 경로는 언제나 이 규칙이 유일하다.
#      ※ 현재 VI_UPPER_ENTRY_BLOCK_ENABLED=False라 이 규칙 자체가 꺼져 있다.
#
...
#
# 🔴 (2026-08-13 사용자 지정) **0.002 -> 0.010.** '본전'이 아니라 **+1% 확보**로.
# [근거 ① 실거래 전수 — 시뮬레이션 아님] 본전스톱 발동 **12건**:
#     고점 순 평균 **+1.85%**  ->  실현 순 평균 **+0.07%**  =  **1.78%p 반납**
#     실현 +1.0% 미만 **12/12건** / 실현 마이너스 **4/12건**
#     (제주반도체 +2.09 -> -0.08 / 포톤 +2.86 -> -0.04 / 주성 +1.64 -> -0.01)
#   즉 바닥 +0.2%는 '본전'이 아니라 **본전 언저리 아무 데나**였다. 딴 이익을
#   통째로 반납하고 심지어 마이너스로 나간다.
# [근거 ② 격자] 실제 매수 59건 + 실제 틱 경로 + **실물 on_price_update**로
#   결정론 확보(같은 설정 2회 실행 결과 동일 확인) 후 무장 x 바닥 2차원 스윕:
#       무장\바닥    0.2%     0.5%     1.0%     1.5%
#       1.0%       +0.534   +0.587     --       --
#       1.5%       +0.589   +0.556   +0.191     --
#       2.0%       +0.627   +0.579   +0.574   +0.376
#       **2.5%     +0.643   +0.651  **+0.698** +0.650**   <- 2.5/1.0이 격자 최고
#   · 날짜별 3승 1패 4무. 59건 중 **6건만** 달라진다(5건 개선).
#   · 🔴 **무장 문턱을 낮추면 4개 열 전부에서 나빠진다**(2.5 > 2.0 > 1.5 > 1.0).
#     문턱을 낮추면 더 많이 무장하고 바닥이 **승자를 자른다** — 실측:
#     조건검색 +6.05 -> +1.00 / 서산 +4.39 -> +0.77 / 다스코 +2.83 -> +0.64.
#     08-10에 문턱을 올린 근거("허용 되돌림 0.8%p로는 상승 초입에 털린다")가
#     1.0%p에서도 그대로 재현됐다. -> **문턱은 2.5% 그대로 두고 바닥만 올린다.**
#     허용 되돌림 2.3%p -> **1.5%p**.
# ⚠️ **당장은 효과가 거의 없다.** 08-10에 문턱을 +2.5%로 올린 뒤 본전스톱은
#    **한 번도 발동하지 않았다**(08-11~13 청산 57건 중 0건). 순 +2.5%를 찍은
#    건이 32건 중 2건뿐이다. 이 값은 '발동할 때 더 좋은 가격에 나가게' 하는
#    보험이지, 지금 성과를 바꾸는 값이 아니다.
# ⚠️ 검증 한계: 틱 아카이브가 **09:00~09:15만** 담아 실거래 재현율 48%이고,
#    결정론 확보를 위해 동적캡·반등소진을 끈 상태였다. **A/B 비교로만** 읽을 것.
# 🔴 불변식: BREAKEVEN_FLOOR < BREAKEVEN_TRIGGER (같거나 크면 무장 즉시 청산).
...
# ── 확정익절 — 본전스톱 대체 (2026-08-15 신설, 사용자 지정) ──────────────
# 본전스톱(무장 +2.5% -> 바닥 +1.0% 이탈)을 **순 +2.0% 도달 즉시 50% 분할**로
# 갈아끼운다. 50% 분할과 잔량 3% 트레일은 08-14 구조를 그대로 쓴다.
#
# [왜] 본전스톱은 '고점을 찍고 되돌아오는 경로'를 판정하는데, 실측상 그 판정이
#   **곧 크게 갈 종목을 정확히 골라 +1%에 파는** 형태였다. 종일 분봉 반사실
#   (실매수 150건 / 8일, 09:00~15:20)에서 발동 건만 떼어 보면:
#       현행(2.5/1.0) 평균 +0.72%  vs  본전스톱 OFF +1.35%  (비관 가정)
#   되돌아온 종목 상당수가 그 뒤 **익절캡(6%)까지** 갔다 —
#   대한광통신 현행 -1.32% / OFF +7.62%, 알트 +4.52% / +9.22%,
#   레몬헬스케어 -0.23% / +6.64%, 마키나락스 +3.74% / +6.78%.
#   08-10에 기록된 이 프로젝트의 근본 문제("판 자리에서 더 간다")와 같은 계열이다.
#
# [실측] 150건 / 8일, 비관·낙관 두 가정, 슬리피지 0.2% 적용(문턱 매도는 분봉
#   '터치=체결' 가정이 구조적으로 낙관적이라 불리하게 태웠다):
#   | 정책                    | 비관   | 낙관   | 승률 | 중앙값 |
#   | 현행 본전스톱 2.5/1.0     | 0.326 | -0.240 | 55% | 0.137 |
#   | **이 안 (+2% 50%분할)**  | 0.360 | -0.046 | 71% | 0.864 |
#   날짜별 비관 5승 3패 / 낙관 6승 2패 — **두 가정 모두에서 현행을 이긴다.**
#   (08-11 물타기 백테스트는 여기서 부호가 뒤집혀 판정 불가였다. 이번엔 안 뒤집혔다.)
#
# ⚠️ **같은 구조에서 문턱만 올리면 더 낫다** — +2.5%는 비관 0.486 / 낙관 0.148로
#   이 안보다 위다. 2.0%는 사용자 지정값이고, 승률·하락일 방어를 사는 선택이다.
# ⚠️ **전량 청산이면 더 낫다**(+2% 전량 비관 0.657 / 낙관 0.719). 50% 분할은
#   성과를 일부 포기하고 상승 참여를 남기는 선택이다(사용자 지정).
# ⚠️ 시뮬레이션은 동적캡·반등소진·VI상단·물타기를 뺀 단순화 사다리다.
#   **절대 수치는 실거래 손익이 아니고 정책 간 비교로만 유효하다.**
# 🔴 `be_partial_done` 가드를 본전스톱과 **공유**한다 — 없으면 분할 후에도
#    순이익이 여전히 문턱 위라 다음 틱에 잔량이 또 팔린다(08-12 VI 결함과 같은 부류).
# 🔴 본전스톱과 **상호 배타**다. True면 BREAKEVEN_* 는 전부 무시된다.
# 롤백: False -> 본전스톱(무장 2.5% / 바닥 1.0%)으로 즉시 복귀.
FLAT_TP_ENABLED = True
FLAT_TP_RATE = 0.020            # 순수익률(수수료 차감 후) 기준
...
# 🔴 가드: 편입가가 현행 2차보다 **낮을 때만** 바꾼다. 편입 후 별로 안 오른
#    종목은 편입가가 오히려 위에 있어(08-12 JW신약: 편입가 2,405 vs 현행 2차
#    2,403) 그대로 쓰면 더 비싸게 산다. -> 2차 **가격**은 언제나 현행 이하다.
#
# 🔴🔴 **2026-08-12 저녁 OFF로 되돌림. 검증이 틀렸다.**
#   위 "체결 90/90 · downside 0"은 **시뮬레이션 결함**이었다. 2차가 미체결일 때
#   `현행 실제 매수가`를 그대로 쓴다고 가정했는데, 실제로는 **1차만 체결되어
#   평단이 올라간다.** 즉 가드는 2차 '가격'만 보장하지 '평단'을 보장하지 않는다.
#   -> 08-12 09:00~09:10 **틱 실측**(n=7):
#        트랜치 체결 13개 -> **11개**, 평단 6,574.1 -> 6,574.9(**+0.008%**),
#        순(MFE-|MAE|) 현행 +8.55%p vs C안 **+8.53%p (현행 우위)**
#      · 닿으면 싸진다: 동양파일 -8.5 / 모나미 -1.5
#      · 안 닿으면 2차가 통째로 날아가 **비싸진다**: 남화토건 **+12.5** /
#        일성건설 +3.0 (남화토건은 현행이면 6,062+6,037 둘 다 잡았다)
#   원래도 기대효과가 +0.08%p로 가장 약했는데 그 근거마저 흔들려 껐다.
#   **교훈: 트랜치 가격을 깊게 바꾸면 '체결률 하락 -> 평단 상승'을 반드시
#   같이 모델링할 것.** 가격만 비교하면 개선처럼 보인다.
# 되살리려면 True — 단 위 미체결 효과를 반영해 재검증한 뒤에.
ENTRY_ANCHOR_SECOND_TRANCHE = False
...
# ══════════════════════════════════════════════════════════════════
# 🔴 추가매수 마감 시각 — 10:00 (2026-08-18 사용자 지정)
# ══════════════════════════════════════════════════════════════════
# "10시 이후에는 무조건 추가매수를 하지 않는다."
#
# 여기서 말하는 **추가매수는 이미 보유한 종목에 물량을 더 얹는 두 경로**다:
#   ① `_maybe_average_down`  — -3% 물타기 (조건 없음, 동일 수량 1회)
#   ② `_rescue_gate`/`_do_rescue_add` — 손절 대신 추가매수 (조건 3개 AND)
# 신규 진입(되돌림 1·2차 트랜치)은 **여기 해당하지 않는다** — 그건 한 종목의
# 최초 1슬롯을 두 번에 나눠 채우는 것이고 120초 타임아웃 안에서만 산다.
#
# [근거 — 2026-08-18 실측]
#  · 그날 물타기 3건이 **전부 손절**로 끝났고 손실 전액을 만들었다:
#      일성건설 09:08 물타기 -> -44,138 / 아진엑스텍 10:44 -> -60,594 /
#      피노 10:57 -> -27,724.  나머지 5건(물타기 없음)은 합계 +35,938원.
#  · 반사실(분봉, 물타기만 제거 · 손절 -4.5%): -96,518 -> **-29,768원**
#    (개선 +66,750). 손절 -5.5% 가정으로도 -43,660원(개선 +52,858).
#  · 08-12~08-18 물타기 15건 중 수동매도로 오염되지 않은 **7건**:
#      승 4건 평균 **+0.80%** (삼화전자/후성/삼기에너지/세아메카닉스)
#      패 3건 평균 **-4.60%** (08-18 3건)
#    -> 손익비가 5.75:1로 불리해 **손익분기 승률이 85%**인데 실측은 57%다.
#       기대값 = 0.57x0.80 + 0.43x(-4.60) = **-1.52%/건**.
#  · 구조적으로도 그렇다 — 물타기는 물량을 2배로 만드는데, 평단이 내려가면
#    확정익절(+2%)·반등소진·본전스톱이 **더 빨리** 걸려 이익은 작게 잘리고,
#    손절선은 2배 물량에 걸린다. 이익은 축소되고 손실만 2배가 되는 구조다.
#
# [왜 '금지'가 아니라 '10:00 컷오프'인가]
#  · 사용자 지정이다. 오전 초반(09:00~10:00)은 되돌림 후 재상승 여지가 있고,
#    실제로 수익으로 끝난 물타기 4건은 **전부 09:01~09:52**에 발동했다.
#    10:00 이후 발동은 실측 2건뿐인데 **둘 다 최대 손실 2건**이었다.
#  · ⚠️ 표본이 얇다(10:00 이후 n=2). 이 값은 데이터가 고른 경계가 아니라
#    **'늦게 물타지 않는다'는 원칙을 시각으로 옮긴 것**이다.
#
# 롤백: 이 값을 time(11, 0)으로 되돌리면 08-12~08-18 사양(물타기 11:00,
#       rescue는 시간 제한 없음)에 가까워진다. 경로별로 따로 되돌리려면
...
# 🔴 추가매수 마감 시각 (2026-08-18 사용자 지정) — 위 ADD_BUY_CUTOFF 참고.
#    08-17까지 이 경로에는 **시간 제한이 아예 없었다**(물타기만 11:00 컷오프가
#    있었다). 즉 14:49에도 손절 대신 추가매수가 가능했고, 그 시점의 추가매수는
#    회복할 시간이 원리적으로 부족하다.
#    컷오프에 걸리면 판정은 그냥 **'sell'(손절)**로 수렴한다 — 안전측이다.
...
#    -> 08-12에 사용자 지정으로 **11:00까지만** 물타기하게 했다.
# 🔴 **2026-08-18에 11:00 -> 10:00으로 앞당겼다**(사용자 지정, 근거는 위
#    ADD_BUY_CUTOFF 주석). 08-18의 10:44·10:57 물타기 2건이 그날 최대 손실
#    2건(-60,594 / -27,724)이었고, 수익으로 끝난 물타기 4건은 전부 09:52
#    이전에 발동했다.
# ⚠️ 이 값은 '판정 시각'이다. 컷오프 이전에 -3%를 찍었더라도 그 순간 다른
#    안전게이트(MDD/지수가드)에 막혀 못 샀으면, 넘긴 뒤엔 안 산다.
...
# 🆕 분할 잔량 하한 (2026-08-18) — 잔량 트레일이 **1차 매도가보다 싸게** 파는
# 구간을 없앤다. 발동가 = max(고점 x (1 - 트레일), 1차 매도가).
#   [기전] 3% 트레일이 1차 매도가보다 높게 팔리려면 고점이 1차가 대비
#     +3.09%(= 1/0.97) 이상이어야 한다. 그러지 못하면 트레일은 **반드시**
#     1차보다 싸게 판다 — 산술적 필연이고 예외가 없다(표본 크기와 무관).
#     확정익절은 순 +2.0%에서 나가는데 거기서 3%를 더 밀어올리는 종목은 드물다.
#   [실측] 08-13~08-18 트레일 청산 5건 중 3건이 1차보다 낮았다:
#     08-18 LB세미콘 1차 5,000 -> 고점 5,060(+1.20%) -> 4,905  (1차 대비 -1.90%)
#     08-14 다날     1차 5,380 -> 고점 5,450(+1.30%) -> 5,280  (-1.86%)
#     08-07 심텍     1차 106,300 -> 고점 109,800(+3.29%) -> 106,100 (-0.19%)
#     반면 고점이 +5.12%였던 STX그린로지스만 +1.86%로 이득이었다.
#   [반사실] 트레일 4건 평균 -0.47% -> +0.47% / 전체 8건 최악 -1.90% -> 0.00%
#     ⚠️ 표본 n=8·3일로 **기준(30건+3일) 미달**이다. 채택 근거는 성과가 아니라
#        위 [기전]의 산술이다.
#   [상방 무영향] 계속 오르면 하한은 영원히 안 걸린다 — 08-18 익절캡 +6.65%,
#     VI 상단 +1.36% 경로는 그대로다.
#   ⚠️ **비용**: 스톱이 더 타이트해진다. 1차가 근처에서 흔들리면 예전보다 일찍
#      나가고 그 뒤 반등분은 못 먹는다(휩쏘). 실거래로 채점할 것.
#   롤백: False -> 08-18 이전 동작(고점 대비 트레일만)
#   🔴🔴 2026-08-18 저녁 — **OFF로 두고 보류한다(반증에서 전제가 깨졌다).**
#     무효 반증이 실틱 1,801건으로 측정한 결과, 분할 잔량이 1차 매도가 아래로
#     내려가기까지 **같은 초(0초)가 91.8% / 5초 이내 97.1%** 였다.
#     즉 하한을 켜면 잔량이 사실상 **즉시 전량 청산**되어, 이 규칙은 실질적으로
#     `BREAKEVEN_EXIT_PARTIAL = False`(전량 매도)와 같아진다 —
#     그것도 **문서화된 스위치가 아니라 옆문으로.**
#     위 [반사실] 수치는 '잔량이 트레일까지 살아남는다'는 전제로 계산한 것인데
#     그 전제가 깨졌다. 하한은 트레일로 나갈 건뿐 아니라 **익절캡·VI로 크게
#     벌던 건까지** 1차가에서 자른다(08-18 LB세미콘 2번: +10,899 -> +2,496 추정).
#     08-18 하루만 봐도 1번 +2,945 / 2번 -7,820으로 **부호가 뒤집힌다.**
#     -> 판정 '불가'. 전량 매도를 원하면 BREAKEVEN_EXIT_PARTIAL을 쓸 것.
#     로직·상수·스위트(test_patch_20260818, 28건)는 롤백용으로 전부 보존한다.
PARTIAL_EXIT_FLOOR_AT_FIRST = False
...
# ── 반등 소진 매도를 '본전 이상'에서만 (2026-08-10 장중, 사용자 지정) ────────
# 원래 이 규칙은 **손실 구간 전용**이었다 — "저점에서 반등했는데 강도·거래량이
# 안 따라오면 -3% 손절을 기다리지 말고 미리 줄인다".
# 그런데 실거래가 두 번 반대로 나왔다:
#   · 08-05: 손실반등 매도 6건 중 **5건이 이후 상승**, 4건은 익절캡까지 갔다.
#            실현 -6,967원 vs 홀딩 시 +28,690원(차이 +35,657원).
#            -> 그때는 '전량 -> 50% 분할'로 절반만 완화했다.
#   · 08-10: 그래도 계속 손실만 확정됐다(GS건설 -0.92% 등).
# 기전은 판정 기준에 있다 — 급락 직후 반등 초기엔 거래량·강도가 **직전 급락
# 구간보다 낮은 게 당연**한데 그걸 '가짜 반등'의 근거로 쓰니 조건이 거의 항상
# 참이 된다. 즉 이 규칙은 손실 구간에서 사실상 상시 발동한다.
# -> True면 **순손익 LOSS_REBOUND_MIN_NET 이상에서만** 발동한다.
#    손실 구간은 -3% 손절에 맡긴다(최후 방어선은 그대로 살아있다).
# ⚠️ 대가: 예전에 -1%대에서 잘리던 건이 이제 **-3%까지 갈 수 있다.**
#    건당 손실은 커지고 대신 회복분을 먹는다. 08-05 실측은 이쪽을 지지한다.
# 롤백: LOSS_REBOUND_REQUIRE_BREAKEVEN = False -> 08-05~08-10 사양(손실 구간 전용).
LOSS_REBOUND_REQUIRE_BREAKEVEN = True
LOSS_REBOUND_MIN_NET = 0.0       # '본전'의 기준 순수익률(0.0 = 수수료 차감 후 본전)
...
class StrategyManager:
    def __init__(
        ...
        # 틱 경로는 분봉을 아예 안 받으므로 캐시가 비어 있으면 시가대비 매수보류
        # (PHASE1A_LEADING_OPEN_SURGE_CAP) 필터를 건너뛴다 — 모르는 값으로 매수를 막지 않는다.
    ...
    def _entry_change_cap(sub_strategy: str) -> float:
        ...
        눌림목 = MAX_ENTRY_CHANGE_PCT_PULLBACK / 그 외(1A) = MAX_ENTRY_CHANGE_PCT.
        (수치를 여기 적지 않는 이유 — 상수를 바꿔도 주석이 안 따라와 거짓말을
         하게 된다. 08-05에 1A 상한을 16% -> 13%로 내렸을 때 실제로 그랬다.)
    ...
    def _entry_change_reject(
        ...
        **새 조건을 넣을 일이 생기면 호출부를 늘리지 말고 여기에만 넣을 것.**
        ...
        """
        # ── 주가 상한 (2026-08-13) — 근거는 MAX_ENTRY_PRICE 주석 참고 ──────
        # 🔴 **전일종가 판정보다 먼저** 본다. 아래 `if not prev_close: return None`
        #    뒤에 두면 전일종가를 모르는 종목에서 상한이 **조용히 같이 죽는다**
        #    (같은 날 `_entry_delay_reject`에서 정확히 이 결합을 결함으로 잡았다).
        # 🔴 이 값은 등락률과 달리 **외부 데이터가 필요 없다** — 가격 하나로
        #    끝나므로 '모름이 매수를 막지 않는다' 규약의 적용 대상이 아니다.
        # ⚠️ 트랜치가는 트리거가의 -0.3~-0.7%라 검사 시점과 체결 시점이 밴드를
        #    넘나들 여지가 사실상 없다(10만원 기준 700원 차이). 08-08 VWAP
        #    사각지대 같은 '통과했는데 못 사는' 구간이 생기지 않는다.
        if MAX_ENTRY_PRICE > 0:
            try:
                _px = float(price)
            except (TypeError, ValueError):
                _px = 0.0
            if _px >= MAX_ENTRY_PRICE:
                return (
                    f"주가 상한 초과 ({_px:,.0f}원 >= {MAX_ENTRY_PRICE:,.0f}원)"
                )
    ...
    def _note_session_vwap(self, stock_code: str, parsed_trade: dict) -> None:
        ...
        try:
            ...
            # ── 단위 상시 감시 (하루 1회) ────────────────────────────
            # 단위는 2026-08-07에 **백만원으로 확정**됐다(상수 주석 참고).
            # 그래도 로그를 남기는 이유는 확인이 아니라 **감시**다 — 키움이
            # 필드 단위를 바꾸면 VWAP이 통째로 어긋나는데, 이 필터는 '조용히
            # 통과'시키는 방향으로 고장 나므로(잘못된 단위면 gap이 거대해져
            # 아무것도 안 막는다) 로그가 없으면 고장을 영영 모른다.
            if not self._vwap_unit_logged:
                ...
                logger.info(
                    ...
                    "-> VWAP=%.1f / 현재가=%s / 비율=%.4f  %s",
                    stock_code, vol, val, vwap, px, ratio,
                    "(정상 — scale=%.0f 유지)" % VWAP_UNIT_SCALE if ok else
                    "(⚠️ 비정상 — 비율이 0.9~1.1을 벗어났다. 키움이 FID14 단위를 "
                    "바꿨을 수 있다. VWAP_UNIT_SCALE 재확인 필요)",
                )
        except Exception:
            pass
    ...
    def entry_delay_sec(self, stock_code: str) -> float:
        """이 종목에 적용할 숙성 요구시간 — **이 규칙의 유일한 지점**.

        (2026-08-07 신설) 조건식별 예외(MIN_ENTRY_DELAY_SEC_BY_COND)를 여기서만
        푼다. 복합 편입("돌파자동매매용+주도주상위")이면 **가장 짧은 값**을
        쓴다 — 돌파가 섞여 있으면 그 종목도 개장 직후에 움직인다고 보는 것이다.

        조건식을 모르면 기본값(MIN_ENTRY_DELAY_SEC)으로 수렴한다. '모름'이
        문턱을 **느슨하게 만들지 않는다** — 여기서만은 보수적으로 간다.
        (등락률·VWAP 게이트의 '모름은 통과' 규약과 방향이 반대인데, 그쪽은
         모르면 매매가 통째로 멈추는 게 문제였고 여기는 반대로 모를 때
         완화되면 [F]의 방어가 조용히 새기 때문이다.)
        """
        if not MIN_ENTRY_DELAY_SEC_BY_COND:
            return MIN_ENTRY_DELAY_SEC
        cond = self._cond_names.get(stock_code) or ""
        if not cond:
            return MIN_ENTRY_DELAY_SEC
        best = MIN_ENTRY_DELAY_SEC
        for part in cond.split("+"):
            v = MIN_ENTRY_DELAY_SEC_BY_COND.get(part.strip())
    ...
    def early_slot_cap_reject(self, now_dt: datetime = None) -> Optional[str]:
        """개장 초반 슬롯 캡 판정 (2026-08-08). None이면 통과, 문자열이면 탈락.

        근거는 EARLY_SLOT_CAP 주석 참고 — 09:05 이전엔 발사 게이트가 없어
        무장만으로 계획이 열리므로, **개수로** 상한을 둔다(가속도 문턱은
        워밍업 아티팩트 때문에 못 막고, 지연시키면 우위가 사라진다).

        ⚠️ 이 판정은 `can_buy_more` 한 곳에서만 부른다 — 두 전략과 두 진입
        경로가 전부 그 함수를 지나므로 여기서 닫으면 새는 곳이 없다.
        호출부를 늘리지 말 것(이 코드베이스의 반복 사고 패턴).
        """
        if not EARLY_SLOT_CAP_ENABLED:
            return None
        now_dt = now_dt if now_dt is not None else self._now()
        if now_dt.time() >= EARLY_SLOT_CAP_UNTIL:
            return None
        held = self.occupied_slots()
        if held < EARLY_SLOT_CAP:
            return None
        return (
            f"개장초반 슬롯 캡 ({held}/{EARLY_SLOT_CAP}, "
            f"{EARLY_SLOT_CAP_UNTIL:%H:%M} 이전)"
        )

    def can_buy_more(self, info: dict | None = None, sub_strategy: str | None = None) -> bool:
        if self._entry_block_reason():
            return False
        # 개장 초반 캡 — 확장 슬롯(7~8)보다 **먼저** 본다. 초반에 확장까지
        # 열리면 캡을 둔 의미가 통째로 사라진다.
        if self.early_slot_cap_reject():
            return False
        held = self.occupied_slots()
    ...
    _REJECT_RULES = (
        ...
        ("거래대금 가속 미달", ("거래대금 가속",)),
        # (2026-08-08) 개장 초반 슬롯 캡. '슬롯 부족'과 **반드시 분리**한다 —
        # 전자는 '자리가 없다', 후자는 '자리는 있는데 일부러 안 쓴다'로
        # 성격이 정반대다. 뭉개면 캡이 과한지 판단할 근거가 사라진다.
        ("개장초반 슬롯 캡", ("개장초반 슬롯 캡",)),
        ...
        # (2026-08-13) 주가 상한. 등락률과 축이 다르므로 반드시 따로 센다 —
        # 뭉치면 "비싸서 걸렀는지" "너무 올라서 걸렀는지"를 구분할 수 없다.
        # ⚠️ 아래 "매수 컷오프"가 부분문자열 '상한'을 잡으므로 그보다 먼저 와야 한다.
        ("주가 상한 초과(고가 종목)", ("주가 상한",)),
    ...
    def _evaluate_1a_pullback_entry(
        ...
        # 슬롯/등락률 상·하한(_entry_change_reject)/익절 캡/시간창.
        #
    ...
    def prearm_candidate(self, stock_code: str, current_price: float = 0.0,
        ...
        버스트는 몇 초 만에 지나가므로 준비는 전부 미리 해둔다:
          ...
          ② 전일종가 캐시 예열 — 등락률 상·하한(_entry_change_reject) 체크에 쓰이는데,
             캐시가 비어 있으면 **주문 직전에** REST(ka10001)를 부른다.
             하필 가장 급한 순간에 0.6초 스로틀 + 429면 2초 blocking이 붙는다.
          ③ 당일 시가 캐시 예열 — 1A 시가대비 매수보류
             (PHASE1A_LEADING_OPEN_SURGE_CAP) 판정용.
            ...
            if not can_buy:
                ...
                if not can_buy:
                    ...
                    self._note_reject(
                        ...
                        # 개장초반 캡은 '슬롯 부족'과 **다른 상태**다(자리가
                        # 남았는데 일부러 안 쓰는 것). 뭉개면 장중에 "왜 6칸
                        # 중 4칸만 쓰나"를 진단으로 판단할 수 없다.
                        or self.early_slot_cap_reject()
                        or (
    ...
    def on_price_update(self, stock_code: str, current_price: float):
        ...
        # 1-b) 본전스톱 (2026-08-03) — 순 BREAKEVEN_TRIGGER(**+2.5%**, 2026-08-10에
        # +1.0%에서 상향)를 한 번이라도 찍은 포지션은 그 뒤 본전 아래로 내려오면
        # 청산한다. '조기확정'이 하던 하방 보호를 이쪽이 대신 맡고, 상단은 캡까지
        # 열어둔다. ⚠️ 문턱은 **익절캡 상향과 세트**다 — 캡만 올리면 08-05의
        ...
        _be_partial = False
        # ── 확정익절 (2026-08-15) — 본전스톱과 **상호 배타** ─────────────
        # 순 FLAT_TP_RATE 도달 **즉시** 50% 분할. 되돌아오길 기다리지 않는다.
        # 근거·한계는 FLAT_TP_ENABLED 주석 참고. 분할 기계(_be_partial ->
        # 아래 exit_reason 블록)와 `be_partial_done` 가드를 본전스톱과 공유한다.
        # 🔴 `partial_exited` 가드가 반드시 필요하다. 없으면 동적캡·VI 상단·
        #    반등소진이 남긴 **잔량**도 순 +2%를 넘는 순간 확정익절이 전량
        #    정리한다 — 08-12에 결함으로 고친 "분할 잔량이 다음 틱에 털린다"와
        #    **같은 결과**이고(경로만 다르다), 이 안을 채택한 백테스트도
        #    '잔량은 트레일이 받는다'로 모델링했다. 가드가 없으면 실거래가
        #    검증한 것과 다른 동작을 한다.
        #    -> 확정익절은 **아직 한 번도 분할되지 않은 포지션에만** 손댄다.
        if FLAT_TP_ENABLED:
            if (exit_reason is None and not pos.get("be_partial_done")
                    and not pos.get("partial_exited")
                    and net_rate >= FLAT_TP_RATE):
                exit_reason = (
                    f"확정익절 {FLAT_TP_RATE*100:.1f}% (순 +{net_rate*100:.2f}%, "
                    f"가격 +{gross_rate*100:.2f}%)"
                )
                _be_partial = True
        elif (BREAKEVEN_STOP_ENABLED and exit_reason is None
        ...
        if (
            ...
            pos["trail_peak"] = peak
            # 🆕 2026-08-18 — 잔량 하한. 1차 매도가 아래로는 트레일로 팔지 않는다.
            #   p <= max(트레일가, 1차가)  ==  (p <= 트레일가) or (p <= 1차가)
            #   🔴 기존 트레일 조건식은 **한 글자도 바꾸지 않고** OR만 얹는다 —
            #      peak*(1-TRAIL) 형태로 고쳐 쓰면 수학적으로는 같아도 부동소수
            #      경계에서 1 ULP가 달라진다(이 프로젝트가 두 번 밟은 함정).
            _first_px = float(pos.get("partial_exit_price") or 0.0)
            _floor_hit = bool(
                PARTIAL_EXIT_FLOOR_AT_FIRST
                and _first_px > 0
                # 🔴 반드시 strict `<` 여야 한다. `<=`로 두면 1차 분할이 바로 그
                #    가격에서 났으므로 **같은 가격 틱에서 잔량이 즉시 털린다** —
                #    08-12에 VI 상단에서 잡았던 '분할 직후 다음 틱 전량청산' 결함과
                #    같은 것이다. 실제로 이 패치 첫 판이 `<=`였고 기존 스위트 3개
                #    (#4 / #18 / #19)가 잡아냈다.
                and current_price < _first_px
            )
            if peak > 0 and (
                (peak - current_price) / peak >= PARTIAL_EXIT_TRAIL
                or _floor_hit
            ):
                exit_reason = (
                    f"분할 잔량 트레일 (고점 {peak:,.0f} 대비 "
                    f"-{(peak - current_price) / peak * 100:.2f}%, 순 {net_rate*100:+.2f}%"
                    + (f", 1차가 {_first_px:,.0f} 하한" if _floor_hit else "")
                    + ")"
                )
        ...
        if exit_reason:
            # ── 본전스톱/확정익절 50% 분할 (2026-08-14 신설) ────────────────
            # 근거는 BREAKEVEN_EXIT_PARTIAL 주석 참고. VI 상단·동적캡·반등소진이
            # 이미 쓰는 기계(PARTIAL_EXIT_FRACTION/TRAIL)를 그대로 재사용한다.
            # 🆕 (2026-08-15) `_be_partial`은 이제 **본전스톱 또는 확정익절**이
            #    세운다. 둘은 상호 배타이므로 여기서 구분할 필요가 없고,
            #    `BREAKEVEN_EXIT_PARTIAL`이 두 경로 공용 '분할 on/off' 스위치다
            #    (False로 두면 확정익절도 전량 청산이 된다 — 실측상 전량이 성과는
            #     더 낫고, 분할은 상승 참여를 남기는 선택이다).
    ...
    def _rescue_gate(self, stock_code: str, pos: dict, current_price: float,
        ...
        # 🔴 추가매수 마감 시각 (2026-08-18 사용자 지정, 기본 10:00).
        #    물타기와 **같은 시각**을 쓴다 — 사용자 지정은 "10시 이후에는
        #    무조건 추가매수 금지"이지 "물타기만 금지"가 아니다.
        #    관찰 중이었더라도 여기서 끊는다(관찰 창은 15초라 손실은 미미하고,
        #    '추가매수 안 함'이 항상 안전측이다). 판정은 손절로 수렴한다.
        if self._now().time() >= RESCUE_ADD_CUTOFF:
            if pos.get("rescue_watch_until") is not None:
                logger.info("[%s] 관찰 중 추가매수 마감(%s) 경과 -> 손절",
                            stock_code, RESCUE_ADD_CUTOFF.strftime("%H:%M"))
            else:
                logger.info("[%s] 추가매수 미발동 — 마감 시각(%s) 경과",
                            stock_code, RESCUE_ADD_CUTOFF.strftime("%H:%M"))
            return "sell"
    ...
    def _update_dynamic_caps(self):
        ...
        for code in list(self.holdings.keys()):
            ...
            if loss_rebound and not cap_exit:
                ...
                # (2026-08-10) 본전 이상에서만 발동하도록 바뀌었으므로 사유 문구도
                # 같이 정정한다 — 예전 문구("손실 축소")는 이제 사실이 아니다.
                _tail = ("본전 위에서 반등 소진" if LOSS_REBOUND_REQUIRE_BREAKEVEN
                         else "손절 대기 대신 손실 축소")
                reason = (
                    f"반등소진 매도 (저점 대비 +{bounce:.2f}% 반등 후 "
                    f"체결강도 {entry_s:.0f}->{cur_s:.0f}, 거래량 x{vol_ratio:.2f}, "
                    f"순 {net_rate:+.2f}% — {_tail})"
                )
    ...
    def _is_loss_rebound_exit(self, pos: dict, price: float, now_dt) -> bool:
        ...
        if not buy_price or not price:
            return False
        _net = self._net_rate(buy_price, price)
        if LOSS_REBOUND_REQUIRE_BREAKEVEN:
            # (2026-08-10 사용자 지정) **본전 이상에서만** 발동한다.
            # 손실 구간은 -3% 손절이 담당한다 — 상수 주석의 실측 근거 참고.
            if _net < LOSS_REBOUND_MIN_NET:
                return False
        elif _net >= 0:
            return False  # (구 사양) 손실 구간에서만 작동(이익 구간은 익절캡 담당)
    ...
    def _execute_sell(self, stock_code, current_price, exit_reason, sell_qty=None):
        ...
        try:
            ...
            if is_partial:
                ...
                # 🆕 2026-08-18 — 잔량 트레일의 하한이 될 '1차 매도가'.
                #   여기가 **모든 분할매도의 단일 창구**다(확정익절 / 동적캡 /
                #   VI 상단 / 반등소진 4경로가 전부 이 함수를 지난다).
                #   ⚠️ **첫 분할만** 기록한다 — 2차가 덮으면 하한이 계단식으로
                #      내려가 보호가 통째로 사라진다.
                if not pos.get("partial_exit_price"):
                    pos["partial_exit_price"] = float(current_price)
