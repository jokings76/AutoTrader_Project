...
# (2026-08-05 저녁 사용자 지정) 2.50 -> 2.00. 고가주 발화 지연을 줄이기 위함.
...
#      MAX=2.0 -> 35,264원~   MAX=2.5 -> 52,909원~
#    즉 2.0은 **35,264원 이상**에만 효과가 있다. 08-05에 실제로 발화가 지연된
#    마키나락스(25,900원 x1.69)·GS건설(29,750원 x1.82)은 **둘 다 이 아래라
#    이 변경의 영향을 받지 않는다.** 그 두 종목까지 덮으려면 MAX<=1.6이어야
#    하는데, 그러면 2.4만원 이상 전 구간이 한 값으로 뭉개져 스케일의 의미가
#    사라진다. 근본 해결은 MAX가 아니라 ALPHA를 낮추는 쪽이다(다만 그러면
#    저가주 교정 효과도 같이 약해진다 — 08-05 실측상 ALPHA 0.40에서는
#    밴드 편차가 9.8배로 오히려 나빠졌다).
BURST_PRICE_MAX = 2.00        # 상한 -> 약 35,264원 이상은 전부 8천만/2억으로 수렴
