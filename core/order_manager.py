...
# 실전 예수금 1,000만원 기준 노출:
#   평상시 슬롯 6 x 50만 = 300만원 (예수금의 30%)
#   확장 슬롯 8 + tier 최대 1.5배(PHASE1A_SIZE_MAX_MULT) = 8 x 75만 = 600만원 (60%)
# 실측상 동시보유는 최대 2/6(08-03)이라 실사용은 100~150만원 수준이다.
# ⚠️ 슬롯 개수·tier 배수·MDD 한도는 **모의와 동일하게 유지**한다(사용자 지정) —
#    이 값 하나만 바꿔야 실전/모의 결과를 같은 축에서 비교할 수 있다.
BUY_AMOUNT_PER_STOCK = 500_000
