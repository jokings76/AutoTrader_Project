...
# ── 15:10 전량 강제청산 ON/OFF (2026-08-12 사용자 지정으로 **OFF**) ──────
# 🔴 이 봇은 08-11까지 **당일청산 전제**로 설계돼 있었다. 사용자가 "15:10
#    강제청산은 없는걸로 — 내가 수동매도하던지 오버나이트 할 수 있게"라고
#    지정해 시간 기반 자동청산을 전면 제거했다.
#    남는 청산은 **가격 기반뿐**이다(손절 / 익절캡 / 본전스톱 / VI 상단 /
#    반등소진 / 물타기 후 최종선). 즉 그 어느 것도 안 걸린 종목은
#    **장을 넘겨 다음 날로 간다.**
#
# ⚠️ 같이 움직이는 것이 3곳 더 있다 — 하나만 바꾸면 조용히 어긋난다:
#    ① strategy_manager.INDEX_GUARD_FORCE_CLOSE_ENABLED (지수가드 14:50 청산)
#    ② strategy_manager.OVERNIGHT_RESTORE_AS_MANUAL (다음날 복원분 격리)
#    ③ main.task_force_close_watcher (여기가 False면 즉시 완료로 표시해
#       자동종료를 막지 않는다 — 안 하면 15:40 하드 폴백까지 봇이 산다)
#
# ⚠️ **오버나이트 리스크는 손절로 막을 수 없다.** 갭 하락은 손절선(-4.5%)을
#    건너뛰고 시가에 체결된다. 그리고 보유분이 매일 쌓이면 예수금이 잠겨
#    신규 매수가 막힌다 — 사용자가 수동으로 정리하는 것이 전제다.
# 롤백: True 한 줄이면 08-11까지의 당일청산으로 완전 복귀한다.
