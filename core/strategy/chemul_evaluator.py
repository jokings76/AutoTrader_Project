...
# phase1b_controller.py의 배선(self.evaluator)이 제거됐고, 지금 이 파일을
# import하는 라이브 코드는 하나도 없다.
#
# 이 FSM 자체가 진입 지연의 큰 축이었다 — 413630 실사례에서 확증게이트에
# 도달하기 전 FSM 단계에서만 19분이 걸렸다(총 지연 20분 중 대부분).
# 게다가 2단계가 의존하는 WallDetector는 원리상 백테스트 검증이 불가능하다
# (호가 잔량 이력을 어디서도 못 구한다 — wall_detector.py 배너 참고).
#
# ⚠️ ChemulState는 한때 strategy_manager.py가 import했지만 2026-08-02에
# 그 import도 제거됐다. 이 enum을 새로 참조하지 말 것.
#
