...
#
# 1B 전략이 이날 삭제되면서 **고아가 됐다**. 유일한 사용처였던
# phase1b_controller.py의 배선이 제거돼, 지금은 형제 파일
# chemul_evaluator.py만 이 파일을 import한다(그쪽도 같이 고아다).
#
# 애초에 진입 판정에서는 2026-07-31에 이미 빠졌다. 이유 두 가지:
#   ① 파라미터(detect_multiplier=5.0 / shrink_ratio=0.7 /
#      disappear_ratio=0.2)가 도입 시점부터 "실데이터로 튜닝 예정"이라고
#      적힌 placeholder였고 한 번도 튜닝된 적이 없다(아래 원문 주석 참고).
#   ② 호가 잔량 이력은 어느 공급자(대신증권 포함)로도 과거 조회가
#      불가능하다 — 거래소가 소매용으로 아카이브하지 않는다. 즉 이 필터는
#      **원리상 백테스트 검증이 영원히 불가능**하다.
#
