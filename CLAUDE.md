...
- 2026-07-27 (계속) — 실거래 크래시 진단 및 수정:
  ...
  - 남은 원인불명 잔여 이슈: 배치파일 실행 시 "지정된 경로를 찾을 수 없습니다"
    한 줄이 맨 위에 뜨는 현상 — 파일 내용/인코딩/AutoRun 레지스트리/파일
    연결/Zone.Identifier 전부 확인했으나 원인 특정 못 함. 이후 실행에는
    전혀 영향 없음(그 뒤로 모든 로그 정상 출력) 확인됨 — 기능적으로 무해,
    보류 상태.
...
1. **종가베팅 스캐너 `evaluate_closing_bet_candidate` 구현** (다음 세션 시작
   시 바로 진행하기로 함): 대상은 `strat.explosion_scorer._cache`에 있는
   종목 전체(하루종일 조건검색에 걸렸던 종목 자동 포함). 설계 합의안 —
   14:50 시점 기준 최근 3분봉 5개(15분)를 보고, 기존 `ExplosionConfig`의
   HIGH(50점)/WATCH(25점) 체계를 그대로 재사용해 `score_bin()` 평균을
   closing_score로, 5개 중 최대 ratio를 surge_ratio로, 양봉 비율을
   bullish_ratio로 계산. eligible = closing_score >= 25(WATCH 기준) AND
   bullish_ratio >= 0.5(사용자 확정 필요, 유일하게 새로 정한 숫자).
   `core/explosion_scorer.py`에 함수 추가하면 됨(재료는 다 있음).
2. **새 전략 구조(1A/Pullback/1B/1L) 실전 검증 전체** — 2026-07-27에 전면
   재설계했고 단위 테스트는 다 통과했지만 실제 장중 데이터로는 아직 미검증.
   특히 1A(거래량증가지속 필터 + 10:30 점수상향), 1L(2분 유지 조건) 위주로
   확인 필요.
3. VWAP 필터 통과율/게이트별 탈락 로그 관찰 및 임계값 튜닝
4. 슬롯 교체 로직(core/slot_replacement.py) 새 sub_strategy 라벨과 실전
   호환 확인 — 코드상 특정 라벨을 참조하지 않아 위험은 낮지만 미검증.
5. 손절 종목 당일 재매수 차단 정책은 현행 유지(재검토 안 함, 의도된 설계)
6. task_auto_shutdown(15:20 자동종료) 재활성화 여부 결정 필요 — 현재 daily_backtest와
   순서 충돌로 임시 비활성화 상태. 재활성화하려면 15:30 백테스트 이후로 시각을
   옮기거나, 백테스트를 완전히 별도 프로세스로 분리해야 함.
7. 네트워크 단절/복구 방어 로직 실전 검증 (2026-07-27 구현 완료, 아직 실제 단절
   상황에서 검증 안 됨 — on_disconnect/on_reconnect 콜백, 격리기간, 미체결 재확인이
   실제 순단/장기단절 상황에서 의도대로 동작하는지 확인 필요)
8. theme_manager.py의 save_themes_to_db() DB 저장 실패("the query contains
   more than one '%s' placeholder") — execute_values 사용법 오류로 추정,
   매매엔 영향 없지만 테마 데이터가 DB에 계속 안 남고 있음(별도 백그라운드
   작업으로 이미 플래그해둠, task_id 세션 종료로 유실됐으면 재플래그 필요).
